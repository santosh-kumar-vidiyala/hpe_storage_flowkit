...
| Method | Purpose |
|--------|---------|
| `create_volume(name, cpg, size, size_unit, params=None)` | Create a new virtual volume. |
| `get_volume(name)` | Retrieve details for a volume. |
| `modify_volume(name, volume_mods, app_type=None)` | Update attributes (e.g. provisioning, userCPG). |
| `grow_volume(name, operation, params)` | Increase capacity (operation + params determine growth). |
